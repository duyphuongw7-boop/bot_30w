...
def init_historical_data():
    ...
    intervals = {'15m': Client.KLINE_INTERVAL_15MINUTE, '30m': Client.KLINE_INTERVAL_30MINUTE, '1w': Client.KLINE_INTERVAL_1WEEK}
...
def start_websockets():
    ...
    ws_manager.start_kline_futures_socket(callback=lambda m: process_ws_kline(m, '15m'), symbol=SYMBOL, interval=Client.KLINE_INTERVAL_15MINUTE)
    ws_manager.start_kline_futures_socket(callback=lambda m: process_ws_kline(m, '30m'), symbol=SYMBOL, interval=Client.KLINE_INTERVAL_30MINUTE)
...
def execute_trade(side, price, reason, ind_data, is_closing=False, amt_to_close=0):
    try:
        ...
        if not is_filled:
            ...
            return # DỪNG HÀM NGAY LẬP TỨC ĐỂ KHÔNG UPDATE SAI VÀO WEB
...
def run_bot():
    ...
    sync_position_from_api() # Lần đầu tiên đồng bộ vị thế khi khởi động
    ...
    while True:
        try:
            ...
            current_price = float(live_data['15m']['close'].iloc[-1])
            ...
            if current_pos != "NONE":
                ...
                b_30m = (inds['30m']['macd'] > inds['30m']['macd_prev']) and (inds['30m']['macd'] >= 0 or inds['30m']['k'] > inds['30m']['d'])
                s_1w = (inds['1w']['macd'] < inds['1w']['macd_prev']) and (inds['1w']['macd'] <= 0 or inds['1w']['k'] < inds['1w']['d'])
                s_30m = (inds['30m']['macd'] < inds['30m']['macd_prev']) and (inds['30m']['macd'] <= 0 or inds['30m']['k'] < inds['30m']['d'])

                if is_long and not (b_1w and b_30m):
                    if current_price <= entry_price and (inds['15m']['lowest_7'] and current_price < inds['15m']['lowest_7']):
                ...
                elif is_short and not (s_1w and s_30m):
                    if current_price >= entry_price and (inds['15m']['highest_7'] and current_price > inds['15m']['highest_7']):
                ...
                c_buy_30m = (inds['30m']['macd'] > inds['30m']['macd_prev']) and (inds['30m']['macd'] >= 0 or inds['30m']['k'] > inds['30m']['d'])
                c_buy_15m = inds['15m']['macd'] > inds['15m']['macd_prev']

                c_sell_1w = (inds['1w']['macd'] < inds['1w']['macd_prev']) and (inds['1w']['macd'] <= 0 or inds['1w']['k'] < inds['1w']['d'])
                c_sell_30m = (inds['30m']['macd'] < inds['30m']['macd_prev']) and (inds['30m']['macd'] <= 0 or inds['30m']['k'] < inds['30m']['d'])
                c_sell_15m = inds['15m']['macd'] < inds['15m']['macd_prev']

                if c_buy_1w and c_buy_30m and c_buy_15m:
                    execute_trade("LONG", current_price, "ĐỒNG THUẬN TĂNG", inds)
                elif c_sell_1w and c_sell_30m and c_sell_15m:
...
async def toggle_kill_switch(req: KillSwitchRequest):
    ...
    if status and amt != 0:
        try:
            ...
            custom_log("🚨 KILL SWITCH ĐĐ ĐÓNG VỊ THẾ HIỆN TẠI THÀNH CÔNG!")
            
            sync_position_from_api() # Cập nhật chuẩn xác vị thế sau khi Kill Switch đóng lệnh
